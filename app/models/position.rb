class Position < ApplicationRecord
    ...
    def self.get_positons(user_id)
        position = Position.execute_sql("select p.salary, p.title, p.start_time ,w.name from position as p ,workspace as w where p.workspace_id=w.id and p.user_id=?",user_id ).to_a
        return position
    end
end
